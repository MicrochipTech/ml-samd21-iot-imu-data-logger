...
int inv_icm426xx_read_dmp_memory(struct inv_icm426xx * s, uint8_t * data, uint32_t offset, uint32_t size)
{
	uint8_t idle;
	uint32_t i;
	uint8_t reg;
	uint8_t memory_bank = 0x40;
	uint8_t dmp_memory_address;

	int status = 0;

	if (size + offset >= 0x1400)
		return INV_ERROR_SIZE;

	status |= inv_icm426xx_read_reg(s, MPUREG_PWR_MGMT_0, 1, &idle);
	reg = idle | ((uint8_t)ICM426XX_PWR_MGMT_0_IDLE_DIS);
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &reg);
	inv_icm426xx_sleep_us(200);

	reg = BIT_DMP_MEM_ACCESS_EN;
	status |= inv_icm426xx_write_reg(s, MPUREG_SCAN0, 1, &reg);
	inv_icm426xx_sleep_us(100);

	memory_bank += (offset / 256);
	dmp_memory_address = offset % 256;

	status |= inv_icm426xx_write_reg(s, MPUREG_MEM_BANK_SEL, 1, &memory_bank);
	status |= inv_icm426xx_write_reg(s, MPUREG_MEM_START_ADDR, 1, &dmp_memory_address);

	for (i = offset ; i < (size+offset) ; i++) {
		/* Handle MEM_BANK_SEL switch on 256 multiples */
		if (0 == (i % 256)) {
			memory_bank = 0x40 + (i / 256);
			dmp_memory_address = 0;
			status |= inv_icm426xx_write_reg(s, MPUREG_MEM_BANK_SEL, 1, &memory_bank);
			status |= inv_icm426xx_write_reg(s, MPUREG_MEM_START_ADDR, 1, &dmp_memory_address);
		}

		status |= inv_icm426xx_read_reg(s, MPUREG_MEM_R_W, 1, &data[i - offset]);
	}
	...
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &idle);

	return status;
}

int inv_icm426xx_check_dmp_memory(struct inv_icm426xx * s, const uint8_t * data, uint32_t offset, uint32_t size)
{
	uint8_t idle;
	uint32_t i;
	uint8_t reg;
	...
	for (i = offset ; i < (size+offset) ; i++) {
		...
		/* To prevent bad increment of dmp address in i2c, the following address is forced */
		status |= inv_icm426xx_write_reg(s, MPUREG_MEM_START_ADDR, 1, &dmp_memory_address);
		status |= inv_icm426xx_read_reg(s, MPUREG_MEM_R_W, 1, &readByte);
		dmp_memory_address ++;
...
int inv_icm426xx_write_dmp_memory(struct inv_icm426xx * s, const uint8_t * data, uint32_t offset, uint32_t size)
{
	uint8_t idle;
	uint32_t i;
	uint8_t reg;
	...
	for (i = offset ; i < (size+offset) ; i++) {
		...
		status |= inv_icm426xx_write_reg(s, MPUREG_MEM_R_W, 1, &data[i-offset]);
	}

	/* disable mem access and restore idle bit */
	reg = 0;
	status |= inv_icm426xx_write_reg(s, MPUREG_SCAN0, 1, &reg);
	inv_icm426xx_sleep_us(100);
	status |= inv_icm426xx_write_reg(s, MPUREG_PWR_MGMT_0, 1, &idle);
