 ...
 *  @return 0 in case of success, negative value on error. See enum inv_error
 */
int inv_icm426xx_write_dmp_memory(struct inv_icm426xx * self, const uint8_t * data, uint32_t offset, uint32_t size);

/** @brief  Check DMP memory at offset given in parameter with data pointed by input pointer.
 *  @param[in] data source data address to be compared to DMP memory
 *  @param[in] offset offset in bytes from RAM start address from where data should be read in DMP memory
